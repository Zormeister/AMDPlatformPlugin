...
cpuid_fn(uint32_t selector, uint32_t *result)
{
	...
	kprintf("cpuid_fn(0x%08x) eax:0x%08x ebx:0x%08x ecx:0x%08x edx:0x%08x\n",
...
cpuid_set_cpufamily(i386_cpu_info_t *info_p)
{
	...
	kprintf("cpuid_set_cpufamily(%p) returning 0x%x\n", info_p, cpufamily);
	return cpufamily;
}
...
cpuid_set_cache_info( i386_cpu_info_t * info_p )
{
	...
	kprintf("cpuid_set_cache_info(%p)\n", info_p);
	...
	for (index = 0; cpuid_deterministic_supported; index++) {
		...
		isAMD ? reg[eax] = 4 : reg[eax] = 0x1D;           /* cpuid request 4 */
		reg[ecx] = index;       /* index starting at 0 */
		cpuid(reg);
		kprintf("cpuid(4) index=%d eax=0x%x\n", index, reg[eax]);
		...
		if (type != Lnone) {
			...
			kprintf(" cache_size[%s]      : %d\n",
				cache_type_str[type], cache_size);
			kprintf(" cache_sharing[%s]   : %d\n",
				cache_type_str[type], cache_sharing);
			kprintf(" cache_partitions[%s]: %d\n",
	...
	if (info_p->cpuid_cores_per_package == 0) {
		...
		kprintf(" cache_size[L2U]      : %d\n",
			info_p->cache_size[L2U]);
		kprintf(" cache_sharing[L2U]   : 1\n");
		kprintf(" cache_partitions[L2U]: 1\n");
		kprintf(" linesizes[L2U]       : %d\n",
	...
	for (i = 1; i < sizeof(info_p->cache_info); i++) {
		...
		kprintf(" 0x%02x", info_p->cache_info[i]);
...
cpuid_set_generic_info(i386_cpu_info_t *info_p)
{
	...
	kprintf("cpuid_set_generic_info(%p)\n", info_p);
	...
	if ((strncmp(CPUID_VID_AMD, info_p->cpuid_vendor,
		...
		(cpuid_set_cpufamily(info_p) == CPUFAMILY_INTEL_PENRYN)) {
		kprintf("Running on AMD CPU, grabbing arguments for core count");
		PE_parse_boot_argn("cpuccorecount=", &cc, sizeof(cc));
		isAMD = TRUE;
	} else if ((strncmp(CPUID_VID_INTEL, info_p->cpuid_vendor,
						min(strlen(CPUID_STRING_UNKNOWN) + 1,
			sizeof(info_p->cpuid_vendor)))) ||
	 ...
	 */
	//if (!isAMD) { wrmsr64(MSR_IA32_BIOS_SIGN_ID, 0); }
	cpuid_fn(1, reg);
	//if (!isAMD) { info_p->cpuid_microcode_version =
	...
	//if (!isAMD) { info_p->cpuid_processor_flag = (rdmsr64(MSR_IA32_PLATFORM_ID) >> 50) & 0x7; }
	/*else { */info_p->cpuid_processor_flag = 1;/* };*/
	...
	kprintf(" max_ext             : 0x%08x\n", info_p->cpuid_max_ext);
	kprintf(" vendor              : %s\n", info_p->cpuid_vendor);
	kprintf(" brand_string        : %s\n", info_p->cpuid_brand_string);
	kprintf(" signature           : 0x%08x\n", info_p->cpuid_signature);
	...
	kprintf(" brand               : %d\n", info_p->cpuid_brand);
	kprintf(" features            : 0x%016llx\n", info_p->cpuid_features);
	kprintf(" extfeatures         : 0x%016llx\n", info_p->cpuid_extfeatures);
	kprintf(" logical_per_package : %d\n", info_p->cpuid_logical_per_package);
	kprintf(" microcode_version   : 0x%08x\n", info_p->cpuid_microcode_version);
	...
	if (info_p->cpuid_max_basic >= 0x5) {
		...
		kprintf(" Monitor/Mwait Leaf:\n");
		kprintf("  linesize_min : %d\n", cmp->linesize_min);
		kprintf("  linesize_max : %d\n", cmp->linesize_max);
		kprintf("  extensions   : %d\n", cmp->extensions);
		kprintf("  sub_Cstates  : 0x%08x\n", cmp->sub_Cstates);
	}
	...
	if (info_p->cpuid_max_basic >= 0x6) {
		...
		kprintf(" Thermal/Power Leaf:\n");
		kprintf("  sensor               : %d\n", ctp->sensor);
		kprintf("  dynamic_acceleration : %d\n", ctp->dynamic_acceleration);
		kprintf("  invariant_APIC_timer : %d\n", ctp->invariant_APIC_timer);
		kprintf("  core_power_limits    : %d\n", ctp->core_power_limits);
		kprintf("  fine_grain_clock_mod : %d\n", ctp->fine_grain_clock_mod);
		kprintf("  package_thermal_intr : %d\n", ctp->package_thermal_intr);
		kprintf("  thresholds           : %d\n", ctp->thresholds);
		kprintf("  ACNT_MCNT            : %d\n", ctp->ACNT_MCNT);
		kprintf("  ACNT2                : %d\n", ctp->hardware_feedback);
		kprintf("  energy_policy        : %d\n", ctp->energy_policy);
	}
	...
	if (info_p->cpuid_max_basic >= 0xa) {
		...
		kprintf(" Architectural Performance Monitoring Leaf:\n");
		kprintf("  version       : %d\n", capp->version);
		kprintf("  number        : %d\n", capp->number);
		kprintf("  width         : %d\n", capp->width);
		kprintf("  events_number : %d\n", capp->events_number);
		kprintf("  events        : %d\n", capp->events);
		kprintf("  fixed_number  : %d\n", capp->fixed_number);
		kprintf("  fixed_width   : %d\n", capp->fixed_width);
	}
	...
	if (info_p->cpuid_model >= CPUID_MODEL_IVYBRIDGE || isAMD) {
		/*
		 * Leaf7 Features:
		 */
		cpuid_fn(0x7, reg);
	...
	if (info_p->cpuid_max_basic >= 0x15) {
		...
		kprintf(" TSC/CCC Information Leaf:\n");
		kprintf("  numerator     : 0x%x\n", reg[ebx]);
		kprintf("  denominator   : 0x%x\n", reg[eax]);
	}
...
cpuid_set_info(void)
{
	...
	if (0 != (info_p->cpuid_features & CPUID_FEATURE_VMM) &&
		...
		case CPUFAMILY_INTEL_PENRYN:
			cpuid_set_cache_info(info_p);
			info_p->core_count   = isAMD ? cc : info_p->cpuid_cores_per_package;
				bool useAMDExperimental;
			PE_parse_boot_argn("-ccusecalc4cc", &useAMDExperimental, sizeof(useAMDExperimental));
			if (useAMDExperimental) {
				info_p->core_count = info_p->cpuid_logical_per_package / 2;
			}
	...
	kprintf("cpuid_set_info():\n");
	kprintf("  core_count   : %d\n", info_p->core_count);
	kprintf("  thread_count : %d\n", info_p->thread_count);
	kprintf("       cpu_type: 0x%08x\n", info_p->cpuid_cpu_type);
	kprintf("    cpu_subtype: 0x%08x\n", info_p->cpuid_cpu_subtype);
...
OSDefineMetaClassAndStructors(CPUChecker, IOService);

bool CPUChecker::start(IOService *provider) {
